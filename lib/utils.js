export const labs = [
  ...
  },
  {
    id: 7,
    type: "lab",
    title: "DVWA SQL Injection (Low Security) — Vulnerability Analysis",
    image: "/images/ff-card-sm.png",
    summary:
      "Hands-on DVWA (Low Security) SQL injection lab write-up: column discovery, schema recon, credential extraction patterns, and remediation guidance. Medium write-up: https://medium.com/@oluwasheun9721/hands-on-sql-injection-lab-dvwa-low-security-405daa8ef78a",
    tags: ["SQLi", "DVWA", "Web Security"],
    keywords: ["DVWA", "UNION-based SQLi", "information_schema", "Privilege mapping"],
    stack: [
      "DVWA (Low Security)",
      "UNION SELECT payload crafting",
      "Schema enumeration via information_schema",
      "Remediation via PDO prepared statements"
    ],
    highlights: [
      "Vulnerability discovery: untrusted input is inserted into a dynamic SQL string without sanitization/parameterization.",
      "Step 1 (column count): confirmed exactly two columns using UNION-based payloads.",
      "Step 2 (schema recon): enumerated tables and columns using information_schema.",
      "Step 3 (data extraction): targeted the users table to extract user/password hash values.",
      "Step 4 (permissions/groups): enumerated users_permissions and users_groups structure for authorization mapping.",
      "Hardening: eliminated the root cause via parameterized queries (PDO prepared statements) plus defense-in-depth controls."
    ],
    timeline: [
      { label: "Recon", value: "Determine column count + enumerate tables/columns via information_schema" },
      { label: "Extraction", value: "Extract users data and map permissions/groups" },
      { label: "Hardening", value: "Prepared statements (PDO) + type casting + least-privilege DB account" }
    ],
    outcome:
      "A complete DVWA Low Security SQLi assessment: vulnerability proof, exploitation workflow, and clear architectural remediation steps."
  },
  {
    id: 8,
    type: "lab",
    title: "DVWA SQL Injection (Medium Security) — Evasion & Bypass Techniques",
    image: "/images/ff-card-sm.png",
    summary:
      "Advanced SQL injection on DVWA Medium Security where single quotes are escaped. Uses CHAR() and unhex() encoding to bypass sanitization and achieve full database enumeration. Medium write-up: https://medium.com/@oluwasheun9721/breaking-down-sql-injection-in-dvwa-medium-security-from-authentication-bypass-to-full-database-6712ab94708f",
    tags: ["SQLi", "DVWA", "Web Security", "Encoding"],
    keywords: ["CHAR() encoding", "unhex() bypass", "MD5 hash extraction", "Privilege mapping"],
    stack: [
      "DVWA (Medium Security)",
      "CHAR() / unhex() encoding for WAF bypass",
      "Boolean-based blind SQLi enumeration",
      "Remediation via input validation + parameterized queries"
    ],
    highlights: [
      "Sanitization bypass: ' is escaped to \\' in Medium security, so CHAR(39) or unhex(27) generates single quotes without triggering the escape.",
      "CHAR() encoding: decimal ASCII codes convert strings (e.g., CHAR(117,115,101,114,115) = 'users').",
      "Database enumeration: extracted tables including users, guestbook, accounts, credit_cards, captured_data.",
      "Sensitive data: harvested MD5 password hashes, credit card numbers, captured user agents, IP addresses.",
      "Permission structure: mapped users_permissions, users_groups, users_usergroups for authorization analysis.",
      "Hardening: applied input validation, whitelist filtering, and PDO prepared statements to eliminate the root cause."
    ],
    timeline: [
      { label: "Analysis", value: "Identify escaped characters and map encoding alternatives (CHAR/unhex)" },
      { label: "Enumeration", value: "Boolean-based blind SQLi to confirm structure, then extract data via CHAR()" },
      { label: "Hardening", value: "Input validation + parameterized queries + least-privilege DB account" }
    ],
    outcome:
      "A complete DVWA Medium Security SQLi assessment demonstrating evasion techniques, full exploitation chain, and clear remediation steps."
  }
];
